...
> **📝 Schema Update (2025-11-01)**: Database schema has been simplified to use Chainlit standard schema (`users`, `threads`, `steps`, `elements`, `feedbacks`) + simple `documents`/`chunks` tables. OAuth-only authentication (no passwords, no JWT). See `/specs/001-improve-rag-system/SCHEMA_SIMPLIFICATION.md` for details.
...
1. **Given** a new user visits the application, **When** they click "Login with Google" or "Login with GitHub" (OAuth only), **Then** their account is created and they are logged into a new chat session
...
- **FR-002**: System MUST support OAuth authentication providers (Google, GitHub) for user login
- **FR-003**: System MUST maintain persistent chat sessions across user logins
- **FR-004**: System MUST display conversation history for authenticated users
- **FR-005**: System MUST implement session lifecycle management per Chainlit specifications
...
- **SC-001**: Users can create accounts and log in within 30 seconds using OAuth (Google or GitHub)
